...
相对于 s12 的变更：
  - 使用 threading.Thread 进行后台执行
  - background_tasks 字典用于生命周期跟踪（bg_id, command, status）
  - background_results 字典 + threading.Lock 实现线程安全存储
  - should_run_background: 通过 run_in_background 参数显式请求模型
  - is_slow_operation: 当模型未指定时的备用启发式判断
  - start_background_task: 分派到守护线程，返回后台任务 ID
  - collect_background_results: 收集已完成的任务，以通知形式返回
  - agent_loop: 慢操作 → 后台 + 占位符，注入通知
  - 通知使用 <task_notification> 格式，不重用 tool_use_id

注意：教学代码保持基本的智能体循环，专注于后台任务。
S11 的完整错误恢复（RecoveryState、退避、升级、响应式压缩、备用模型）已省略。
"""
